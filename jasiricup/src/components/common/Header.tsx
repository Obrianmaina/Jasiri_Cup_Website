...
export const Header = () => {
  ...
  return (
    <header className="bg-white dark:bg-gray-900 py-4 relative transition-colors duration-300 z-50">
      <nav className="container mx-auto px-4 flex items-center justify-between">
        ...
        {/* Desktop Navigation */}
        <div className="hidden xl:flex items-center bg-gray-100 dark:bg-gray-800 rounded-full px-6 py-2 space-x-6 transition-colors duration-300">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
        ...
        <div className="hidden xl:flex items-center space-x-3 shrink-0">
          ...
          <Link href="/get-in-touch" className="bg-green-500 text-white px-5 py-2 text-sm rounded-full hover:bg-green-600 transition-colors whitespace-nowrap">
            {t('getInTouch')}
          </Link>
        </div>
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="xl:hidden absolute top-full left-0 w-full bg-white dark:bg-gray-900 shadow-lg z-50 border-t border-gray-200 dark:border-gray-800 transition-colors duration-300">
            ...
            <div className="flex flex-col space-y-2 mb-6">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`text-base font-medium transition-colors duration-200 py-2.5 rounded-lg
                    ${pathname === link.href 
                      ? 'text-gray-800 dark:text-white border-l-4 border-green-500 pl-4 bg-gray-50 dark:bg-gray-800/50' 
                      : 'text-gray-700 dark:text-gray-300 hover:text-purple-600 dark:hover:text-purple-400 pl-5'
                    }
                  `}
                >
                  {link.name}
