...
export default function ProductsClient({ initialProducts }: { initialProducts: IProduct[] }) {
  ...
  const handleAddVariation = () => {
    setFormData({
      ...formData,
      variations: [...(formData.variations || []), { color: '', size: '', stockQuantity: 0, image: '' }]
    });
  };
        ...
        <div className="bg-white dark:bg-gray-900 p-5 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
          ...
          <form onSubmit={handleSave} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Main Image URL</label>
              ...
              {(!formData.variations || formData.variations.length === 0) ? (
                ...
                <div className="space-y-4">
                  {formData.variations.map((v, index) => (
                    <div key={index} className="flex flex-col gap-3 bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700 transition-colors">
                      
                      {/* Top Row: Color, Size, Stock, Remove */}
                      ...
                      {/* Bottom Row: Variant Image */}
                      <div className="w-full">
                        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Variant Image URL (Optional)</label>
                        <input type="text" placeholder="https://res.cloudinary.com/..." value={v.image || ''} onChange={e => handleUpdateVariation(index, 'image', e.target.value)} className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-md px-3 py-1.5 text-sm focus:ring-2 focus:ring-purple-500 focus:outline-none transition-colors" />
                      </div>
                              ...
                              <div className="flex flex-col gap-1">
                                {product.variations.map((v, i) => (
                                  <div key={i} className="text-xs text-gray-600 dark:text-gray-300 flex items-center gap-2">
                                    {v.image && <img src={v.image} alt={v.color} className="w-4 h-4 rounded-full object-cover" />}
                    ...
                    <div key={product._id} className="p-5 space-y-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      <div className="flex justify-between items-start gap-3">
                        <div className="flex gap-3">
                          {product.image && <img src={product.image} alt={product.name} className="w-12 h-12 rounded object-cover" />}
                          <div>
