...
export default function TeamManagementPage() {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/invite', { 
        ...
        body: JSON.stringify({ email, name }), // Removed role dropdown, defaults to 'Admin' on backend
      });
      const data = await res.json();
      if (data.success) {
  ...
  // *** UPDATED TO INCLUDE 'toggle-finance' ***
  ...
  if (!isMaster) {
    return (
      <div className="py-12 text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2">Only the Master Admin is authorized to access team management.</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto py-8 space-y-8">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Authorize staff, reset 2FA configurations, and manage portal access.</p>
        </div>
      </div>
